...
const VocabularyN2LessonListScreen = () => {
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', async () => {
      const completeId = await AsyncStorage.getItem('lastCompletedLesson_N2')
      if (completeId) {
        ...
        if (idNum === 50) {
          const unlockedN1 = await AsyncStorage.getItem('unlockedLessons_N1')
          const parsedN1 = unlockedN1 ? JSON.parse(unlockedN1) : []
          if (!parsedN1.includes(1)) {
            const updatedN1 = [...parsedN1, 1]
            await AsyncStorage.setItem('unlockedLessons_N1', JSON.stringify(updatedN1))
          }
        }

        await AsyncStorage.removeItem('lastCompletedLesson_N2')
      }
    })
    return unsubscribe
  }, [navigation, unlockedLessons])

  const topics = generateLessons(1, 50, navigation as any, 'N2', unlockedLessons)
